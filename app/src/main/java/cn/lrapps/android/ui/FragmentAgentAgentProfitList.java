...
public class FragmentAgentAgentProfitList extends MyBasePageFragment implements IAjaxDataResponse, View.OnClickListener
{
	private static final String TAG = FragmentAgentAgentProfitList.class.getSimpleName();
	...
	public FragmentAgentAgentProfitList()
	{
	}

	public static FragmentAgentAgentProfitList newInstance()
	{
		return new FragmentAgentAgentProfitList();
	}
	...
	private void getTotalMoney(final TextView tv, final byte moneyUnit, boolean isToday)
	{
		...
		agentService.getAgentReferrerBalanceLogMoney(moneyUnit, isToday, "", true);
	}
	...
	synchronized public void loadMoreData()
	{
		...
		mUserBalanceLogInfoService.getAgentReferrerBalanceLogInfoList(mDataStart, getPageSize(), tips, true);
	}
	...
	public boolean onAjaxDataResponse(String url, String result, AjaxStatus status)
	{
		...
		if (url.endsWith(ApiConfig.GET_AGENT_REFERRER_BALANCE_LOG_INFO_LIST))
		{
