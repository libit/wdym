...
import cn.lrapps.android.ui.adapter.FansAdapter;
...
public class FragmentFansList extends MyBasePageFragment implements IAjaxDataResponse
{
	private static final String TAG = FragmentFansList.class.getSimpleName();
	...
	public FragmentFansList()
	{
	}

	public static FragmentFansList newInstance()
	{
		return new FragmentFansList();
	}
	...
	{
		View rootView = inflater.inflate(R.layout.fragment_fans_list, container, false);
	...
	synchronized public void loadMoreData()
	{
		...
		mUserService.getUserFansList(null, null, mDataStart, getPageSize(), tips, true);
	}
	...
	synchronized private void refreshUserInfoList(List<UserInfo> userInfoList)
	{
		...
		if (mFansAdapter == null)
		{
			final Context context = FragmentFansList.this.getContext();
			FansAdapter.IItemClick iItemClick = new FansAdapter.IItemClick()
			{
				@Override
			...
			mFansAdapter = new FansAdapter(context, mUserInfoList, iItemClick);
			xListView.setAdapter(mFansAdapter);
		}
		else
		{
			mFansAdapter.notifyDataSetChanged();
