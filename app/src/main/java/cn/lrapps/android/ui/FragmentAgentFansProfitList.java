...
public class FragmentAgentFansProfitList extends MyBasePageFragment implements IAjaxDataResponse, View.OnClickListener
{
	private static final String TAG = FragmentAgentFansProfitList.class.getSimpleName();
	...
	public FragmentAgentFansProfitList()
	{
	}

	public static FragmentAgentFansProfitList newInstance()
	{
		return new FragmentAgentFansProfitList();
	}
	...
	private void getTotalMoney(final TextView tv, final byte moneyUnit, boolean isToday)
	{
		...
		agentService.getAgentFansBalanceLogMoney(moneyUnit, isToday, "", true);
	}
	...
	synchronized public void loadMoreData()
	{
		...
		mUserBalanceLogInfoService.getAgentUserFansBalanceLogInfoList(mDataStart, getPageSize(), tips, true);
	}
	...
	public boolean onAjaxDataResponse(String url, String result, AjaxStatus status)
	{
		...
		if (url.endsWith(ApiConfig.GET_AGENT_USER_FANS_BALANCE_LOG_INFO_LIST))
		{
