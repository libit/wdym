...
public class ActivityUserAuth extends MyBaseActivity implements View.OnClickListener, IAjaxDataResponse
{
		...
		switch (v.getId())
		{
			case R.id.iv_photo:
			{
				...
				else
				{
					...
					startActivityForResult(new Intent(ActivityUserAuth.this, ImageGridActivity.class), IMAGE_PICKER);
				}
				break;
			}
			...
			case R.id.btn_submit:
			{
				...
				mUserService.updateUserAuthInfo(name, number, code, sex, identifityNumber, province, city, picUrl, "正在提交...", true);
				break;
			}
		}
	}
	...
	{
		if (url.endsWith(ApiConfig.UPDATE_USER_AUTH_INFO))
		{
			ReturnInfo returnInfo = GsonTools.getReturnInfo(result);
			if (ReturnInfo.isSuccess(returnInfo))
			{
				ToastView.showCenterToast(this, R.drawable.ic_done, "提交实名认证资料成功！");
				EventBus.getDefault().post(UserEvent.CHANGED_DATA);
				finish();
			}
			else
			{
				String msg = "提交实名认证资料失败。";
				if (returnInfo != null)
				{
		...
		//					etNumber.setText(mUserInfo.getEmail());
		//					final List<String> sexStringList = new ArrayList<>();
		//					sexStringList.add("男");
		//					sexStringList.add("女");
		//					sexMap.put("男", SexType.MALE.getSex());
