 ...
 * 日志记录类
 */
public class LogTools implements Thread.UncaughtExceptionHandler
{
	...
	public void start()
	{
		...
		if (directory != null && directory.isDirectory())
		{
			...
			if (count > MAX_SAVE)
			{
				...
				LogTools.debug("saveFiles", "saveFiles:" + GsonTools.toJson(saveFiles));
				for (int i = saveFiles.size() - 1; i >= MAX_SAVE; i--)
				{
					if (saveFiles.get(i).isFile())
					{
						LogTools.debug("saveFiles", "delete:" + GsonTools.toJson(saveFiles.get(i)));
	...
	public void stop()
	{
		...
		LogTools.debug(TAG + "stop", "停止记录日志！");
	}
