 ...
 * Copyright (c) 2018.
 */
package com.weiduyx.wdym.wxapi;
...
public class WXEntryActivity extends Activity implements IWXAPIEventHandler
{
	...
	{
		//		Toast.makeText(this, "发送请求：", Toast.LENGTH_LONG).show();
		switch (req.getType())
		{
	...
	{
		//Toast.makeText(this, "收到响应：" + GsonTools.toJson(resp), Toast.LENGTH_LONG).show();
		LogTools.info("onReq", GsonTools.toJson(resp));
		int result = 0;
		...
		switch (resp.errCode)
		{
			case BaseResp.ErrCode.ERR_OK:
			{
				try
				{
					...
					EventBus.getDefault().post(new WxLoginEvent(WxLoginEvent.WX_REQUEST_LOGIN, code, ""));
					tvMsg.setText("正在登录，请稍后...");
				}
				catch (Exception e)
				{
				}
